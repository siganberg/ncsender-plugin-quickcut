...
function makeQuickCutGenerator(isImperial) {
  ...
  function computePolygonGeom(params, offsetX, offsetY, extraRotDeg) {
    ...
    var newApothem = (cutType === 'inner') ? apothem - toolRadius : apothem + toolRadius;
    var pathRadius = newApothem / Math.cos(Math.PI / sides);

    var off = originOffset(origin, 2 * radius, 2 * radius);
    ...
    return {
      ...
      sides: sides,
      toolRadius: toolRadius,
      startAngle: Math.PI / sides - Math.PI / 2 + startDeg * Math.PI / 180
    };
  }
  ...
  function generatePolygonProgram(params) {
    ...
    var extras = [
      ...
      'Radius: ' + params.radius + unitsLabel + ' (circumscribed)',
