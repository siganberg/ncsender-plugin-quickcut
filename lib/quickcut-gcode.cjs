...
function makeQuickCutGenerator(isImperial) {
  ...
  function computePatternPositions(pattern) {
    ...
    if (pattern.style === 'linear' || pattern.style === 'honeycomb') {
      ...
      // Honeycomb: every odd row shifts by xDist/2 (Skadis-style
      // staggered layout — set yDist = xDist for square pitch, or
      // xDist × √3/2 for equilateral hex packing).
      // hexTrim: shifted rows get one fewer item so the pattern's
      // left and right ends align (no dangling row overhang).
      var isHoney = pattern.style === 'honeycomb';
      var hexTrim = isHoney && !!pattern.hexTrim;
      for (var j = 0; j < yCount; j++) {
        var isShiftedRow = isHoney && (j % 2 === 1);
        var rowOffset = isShiftedRow ? xDist / 2 : 0;
        var rowCount = (hexTrim && isShiftedRow) ? Math.max(0, xCount - 1) : xCount;
        for (var i = 0; i < rowCount; i++) {
          positions.push({ x: i * xDist + rowOffset, y: j * yDist, rotDeg: 0 });
        }
      }
      return positions;
    }
